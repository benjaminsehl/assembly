...
3. In interview mode, ask a focused batch of high-leverage questions before judging the idea. Cover user, painful moment, current workaround, urgency, desired outcome, wedge, constraints, distribution, and success evidence. Stop for answers when those choices would materially shape the product.
4. Before recommending a wedge, scope, or success criteria, make sure the conversation or project trail answers what is being built, why it matters, and what good looks like.
5. In delegated mode, state the assumptions you are making, mark recommendations as recommendations, and name which assumptions need later validation.
6. Generate and compare narrow wedges, alternatives, risks, and evidence paths only after the key user/product inputs are clear enough.
7. Apply founder critique when ambition, differentiation, scope, or product delight is the risk.
8. Apply the business-model lens when ICP, pricing, distribution, retention, or cost structure matters.
9. Apply the design-plan lens when the next step is a planned UI or user flow.
10. Produce a discovery brief with user, pain, wedge, alternatives, risks, evidence needed, open questions, and recommended next skill.
11. Stop before `spec` unless the user asks to continue.
...
- What, why, and what good looks like are explicit or named as open questions.
