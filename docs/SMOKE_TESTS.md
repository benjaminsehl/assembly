...
| `introspect` | "Use introspect to tell me what state this project is in and get it back on track." | Deeper audit with conformity verdict, evidence checked, status update or skip reason, recovery plan, next skills, and first action. |
...
- [ ] `project-status` recommends `introspect` when status, phase, or recovery path are unclear.
- [ ] `introspect` updates `docs/status.md` or states why it did not.
